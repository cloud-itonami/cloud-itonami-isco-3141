(ns field-lab-science-support.actor
  "FieldLabScienceSupportActor — the ISCO-08 3141 independent field-
  and-lab-science-support actor as a `langgraph.graph/state-graph`
  (per ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  sampling operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the SamplingAdvisor can never directly
  commit a record or dispatch a robot action the FieldLabGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [field-lab-science-support.advisor :as advisor]
            [field-lab-science-support.governor :as governor]
            [field-lab-science-support.store :as store]))

(defn build-graph
  "Build a compiled FieldLabScienceSupportActor graph. `store`
  implements `field-lab-science-support.store/Store`. `advisor`
  implements `field-lab-science-support.advisor/Advisor` (defaults to
  `mock-advisor`). `checkpointer` defaults to an in-memory one."
